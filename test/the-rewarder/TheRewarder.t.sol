...
import "./utils/TheRewarderFixture.sol";
import "src/src-default/attacker-contracts/AttackRewarder.sol";

contract TheRewarder is TheRewarderFixture {
    AttackRewarder public attackRewarder;
    ...
    function test_theRewarder() public {
        ...
        attackRewarder =
            new AttackRewarder(address(flashLoanerPool), address(token), address(theRewarderPool), attacker);

        vm.warp(block.timestamp + 5 days);

        attackRewarder.attack(TOKENS_IN_LENDER_POOL);
    ...
    function _assertions() internal {
        ...
        for (uint8 i; i < 4; i++) {
            ...
            uint256 awardPerUser = theRewarderPool.rewardToken().balanceOf(users[i]);
        ...
        uint256 awardAttacker = theRewarderPool.rewardToken().balanceOf(attacker);
