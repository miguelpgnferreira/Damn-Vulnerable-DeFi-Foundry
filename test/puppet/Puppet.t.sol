...
contract PuppetTest is PuppetFixture {
    ...
    function test_puppet() public {
        ...
        (, bytes memory data) =
            exchange.call(abi.encodeWithSignature("getEthToTokenOutputPrice(uint256)", tokensToBuyBack));
        uint256 ethToSend = abi.decode(data, (uint256));

        exchange.call{value: ethToSend}(
            abi.encodeWithSignature("ethToTokenSwapOutput(uint256, uint256)", tokensToBuyBack, block.timestamp * 2)
        );
